...
class Attention(nn.Module):
    ...
    def _attn(self, q, k, v, attention_mask=None, head_mask=None, output_attentions=False):
        ...
        if attention_mask is not None:
            batchsize,head_num,msl1,msl2=w.shape
            # Apply the attention mask
            # print(f"---------- the attention mask shape is: {attention_mask.shape}")
            # CMTLZ: shape of w = (2, 12, msl, msl)
            # CMTLZ: shape of attention_mask = (2,1,1,msl*msl)
            w = w + attention_mask.view(batchsize,-1,msl1,msl2)
...
class GPT2Model(GPT2PreTrainedModel):
    ...
    Return:
        ...
        if attention_mask is not None:
            ## edit by liangzi
            ## here we change the past version to new version to support the attention-matrix level attention mask.
            if batch_size <= 0:
                raise ValueError("batch_size has to be defined and > 0")
            bs,sql,sql2=attention_mask.shape
            # assert(bs==batch_size)
            # assert(sql==sql2)

            # print(bs,sql)
            # print(attention_mask.shape)
            attention_mask = attention_mask.view(batch_size,sql,sql)
            ...
            # print("the shape of attention mask is:{}".format(attention_mask.shape))
            
            # print("the attention mask shape is")
            # print(attention_mask.shape)
            # print("==================================")
            ...
            ### here is the past version, i.e. original version in V3.0.2
